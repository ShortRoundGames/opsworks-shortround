...
status_command = "/opt/chef/embedded/bin/bluepill status"
...
if shell.exitstatus == 0
  ...
  bash "restart bluepill" do
    ...
    cwd "/tmp"
    code <<-EOS
      /opt/chef/embedded/bin/bluepill restart node1
    EOS
  end
  ...
  bash "start bluepill" do
    ...
    cwd "/tmp"
    code <<-EOS
      /opt/chef/embedded/bin/bluepill load /mnt/server/current/#{pillName}
    EOS
  end
